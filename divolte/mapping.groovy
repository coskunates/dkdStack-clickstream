mapping {
    ...
    def locationUri = parse referer() to uri
    def locationQuery = locationUri.query()
    map locationQuery.value('search') onto 'searchKeyword'

    def u = parse location() to uri
    section {
        when u.path().equalTo('/home') apply {
    ...
    map eventParameter('href') onto 'href'
    map { parse eventParameter('categoryId') to int32 } onto 'categoryId'
    map { parse eventParameter('productId') to int32 } onto 'productId'
}
